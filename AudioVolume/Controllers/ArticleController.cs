...
namespace AudioVolume.Controllers
{
    ...
    public class ArticleController : Controller
    {
        ...
        #region Showroom
        [ChildActionOnly]
        public ActionResult ListShowroom()
        {
            var showrooms = _unitOfWork.ShowRoomRepository.Get(orderBy: q => q.OrderBy(a => a.Sort));
            return PartialView(showrooms);
        }
        public ActionResult Showroom(string result = "")
        {
            ViewBag.Showroom = result;
            var showroom = new ShowRoom { Sort = 1};
            return View(showroom);
        }
        [HttpPost, ValidateInput(false)]
        public ActionResult Showroom(ShowRoom showroom)
        {
            if (ModelState.IsValid)
            {
                _unitOfWork.ShowRoomRepository.Insert(showroom);
                _unitOfWork.Save();
                return RedirectToAction("Showroom", new { result = "success" });
            }
            return View(showroom);
        }
        public ActionResult UpdateShowroom(int showroomId = 0)
        {
            var showroom = _unitOfWork.ShowRoomRepository.GetById(showroomId);
            if (showroom == null)
            {
        ...
        [HttpPost, ValidateInput(false)]
        public ActionResult UpdateShowroom(ShowRoom model, FormCollection fc)
        {
            var showroom = _unitOfWork.ShowRoomRepository.GetById(model.Id);
            if (ModelState.IsValid)
            {
                showroom.Name = model.Name;
                showroom.Sort = model.Sort;
                showroom.Body = model.Body;
                _unitOfWork.ShowRoomRepository.Update(showroom);
                _unitOfWork.Save();
                return RedirectToAction("Showroom", new { result = "update" });

            }
            return View(showroom);
        }
        [HttpPost]
        public bool DeleteShowroom(int showroomId = 0)
        {
            ...
            _unitOfWork.ShowRoomRepository.Delete(showroom);
            _unitOfWork.Save();
            return true;
        }
        #endregion
