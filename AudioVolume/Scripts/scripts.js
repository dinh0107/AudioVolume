...
function homeJs() {
    ...
    $(".header-nav-new .header-nav-main .header-nav-main-content").addClass('active');
...
$(document).ready(function () {
    ...
    var review_product_search_content = document.getElementById('review-product-search-content');
    var review_pro_search_mobile = document.getElementById('review-product-search-mobile');
    var review_product_search_content_mobile = document.getElementById('review-product-search-content-mobile');


    var close_suggest_search = document.getElementById('close-suggest-search');
    var close_suggest_search_mobile = document.getElementById('close-suggest-search-mobile');
    ...
    keysearch_header_mobile.onblur = keysearch_header_mobile.onfocus = function () {
    ...
    keysearch_header.onkeyup = function () {
        ...
        if (keysearch.length >= 3) {
            $.ajax({
                ...
                console.log(data)
                var x = data.data + '<div class="close-suggest-search" id="close-suggest-search">Đóng</div>';
                review_product_search_content.innerHTML = x;
                document.getElementById('close-suggest-search').onclick = close_all_suggest
            });
        }
    };
    ...
    keysearch_header_mobile.onkeyup = function () {
        ...
        if (keysearch.length >= 3) {
            $.ajax({
                ...
                var x = data.data + '<div class="close-suggest-search" id="close-suggest-search-mobile">Đóng</div>';
                review_product_search_content_mobile.innerHTML = x;
                document.getElementById('close-suggest-search-mobile').onclick = close_all_suggest
            });
        }
    };
});
